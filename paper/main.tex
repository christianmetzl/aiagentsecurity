...
\code{SECRET\_MARKER} sentinel post and free diversity via distinct destination
\emph{domains} are \emph{benchmark-exploitation}: they score the public column, but
posting an opaque token to a benign URL is not a real compromise and ``distinct
destination $\neq$ distinct attack.'' The read-\code{secret.txt}-then-forward and
untrusted-content-then-act routes are \emph{genuine compromises}. We present the sentinel
route as metric-optimisation, not as the security headline (\S\ref{sec:results}).

\paragraph{Beyond fixed routes: an online Go-Explore search.} FORGE's default probe-then-fan
search can only fan out routes it was written to try. We therefore also implement, in the
same \code{attack.py}, an online \textbf{Go-Explore / quality-diversity search}
(\code{mode="explore"}) that uses the live target model as its fitness oracle: it keeps an
archive keyed by the SDK's score-cell, \emph{returns} to a promising cell via
\code{env.restore(snapshot)} and then \emph{explores} by appending one move from a mutation
grammar (read / open / email / post / post-what-you-read / send / shell / follow-injection),
rewarding predicate severity, cell novelty, and a real-attack bonus. This is a faithful
instance of the family FORGE's baseline only gestures at~\cite{ecoffet2021}: it discovers,
rather than assumes, what fires on a specific target. In the offline ablation it grows a
$\sim$400--500 cell archive and finds 15 (public) to $\sim$270 (strict) firing multi-step
chains; because it \emph{explores} rather than \emph{exploits}, its raw score is below
FORGE's fan-out on the mock --- the value is discovery on an unknown target, with the
explore-then-fan-out hybrid the natural combination (and FORGE's probe-then-fan its
...
Table~\ref{tab:mechanism} reports offline scores at a 60-second budget --- far below the
competition's per-model budget (the SDK's default attack budget is 1800\,s), so these are mechanism checks, not
...
\subsection{A genuine multi-step compromise, and what the guardrails do with it}

The security headline is not the sentinel route but a real chain: an agent consults
untrusted web content, reads the protected \code{secret.txt}, and exfiltrates its
\emph{real} credential values. The contrast across guardrails \emph{is} the finding
...
\noindent The \textbf{public guardrail correctly defends the canonical indirect-prompt-injection
attack} --- which is precisely why the only public-scorable route is the sentinel
data-channel gap --- and the genuine compromise leaks only through the strict baseline's
``post''-keyword mismatch (Bug 2, \S\ref{sec:bugs}), which our reference guardrail closes.
This is the durable security content of the work.
...
only on the strict baseline via Bug 2. (v) Offline budgets are short (60\,s), far below the competition's
per-model budget; they validate mechanism, not the real ceiling.
...
SDK; we will disclose them to the organizers before publication. We lead with the defensive contribution
