...
| C14 | Density-packing lifts the offline score over the wide fill on a fast target, adaptively and budget-safely (falls back to wide on a slow target) | fast compliant/optimal: norm 44.9 (off) → 688 (on) @30 s, → 1000 @60 s, 128 posts/candidate; slow: on 10.6 ≥ off 9.6; strict: dense N/A (posts/cand=1); every row replay < budget | `attack.py` (dense fill); `experiments/dense_ablation.py` | `results/dense_ablation.json`; `verify.py` check 1 (norm 575 @25 s); `tests` | Measured-offline | verified |
