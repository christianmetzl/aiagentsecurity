...
| C17 | No genuine guardrail BYPASS exists: neither a shell-read of the secret nor taint-laundering scores past either provided guardrail | shell reads nothing (allows only echo/ls/pwd/whoami/date); guardrail taint window (5 events) ≥ predicate window (2) ⇒ no laundering; sweep finds 0 bypasses | `experiments/guardrail_bypass_search.py` | `results/guardrail_bypass_search.json` | Source-traced + Measured-offline | verified (negative) |
