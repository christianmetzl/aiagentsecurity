...
h('14  Reproducibility');
p(null, 'The attack, the local evaluation harness, the reference defense, a claims ledger (every claim tagged by evidence class), and the build presets discussed here are released with this note. A single command reproduces the mechanism checks against the SDK’s own environment, predicates, scoring, and both packaged guardrails; the full test suite is green. The figures are generated by a committed script from the measured constants and the real-eval log. Development, analysis, and drafting were assisted by Claude Code (Anthropic).');
