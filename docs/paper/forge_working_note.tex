...
8750$s; each candidate costs $L\approx L_{\text{prefill}}+K\,L_{\text{decode}}$, dominated by a
$\approx25$s prefill for a 20B model on the evaluation hardware --- a constant a competitor solution
encodes directly~\cite{lb60}. Because scoring is additive with no interaction, a submission's score is
...
The attack, the local evaluation harness, the reference defense, a claims ledger (every claim tagged by
evidence class), and the build presets discussed here are released with this note. A single command
reproduces the mechanism checks against the SDK's own environment, predicates, scoring, and both
packaged guardrails; the full test suite is green. Figures~\ref{fig:ladder}--\ref{fig:pf} are generated
by a committed script from the measured constants and the real-eval log.
